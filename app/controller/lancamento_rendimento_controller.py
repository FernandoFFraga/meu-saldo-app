...
from app.constants.table_constants import TB_LAC_REN as TBNAME
from app.model.lancamento_rendimento_model import LancamentoRendimento


def insert(item: LancamentoRendimento):
    conn = db.get_conn()
...
def insert_many(item: LancamentoRendimento, count: int):
...
def select_by_id(id: int):
    ...
    with conn.cursor() as cursor:
        ...
        return LancamentoRendimento().set(cursor.fetchone())


def update(item: LancamentoRendimento):
    conn = db.get_conn()
